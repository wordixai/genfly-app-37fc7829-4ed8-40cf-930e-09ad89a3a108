import { ExternalLink, Boxes, Layers, TrendingUp, Shield } from "lucide-react";
  ...
  const features = [
    {
      ...
      visual: (
        <div className="absolute inset-4 grid grid-cols-5 gap-x-0 gap-y-1 scale-105 z-10">
          {/* Provider icons with floating animation */}
          {Array.from({ length: 25 }, (_, i) => (
            <div
              key={i}
              className={`size-9 transform hover:scale-110 hover:brightness-110 transition-all duration-500 ease-out`}
              style={{
                animation: `4s ease-in-out ${i * 150}ms infinite normal none running float${i}`,
                opacity: 0.85,
                transform: i % 5 > 2 ? 'translateX(36px)' : 'none'
              }}
            >
              <div className="flex items-center justify-center size-6 flex-shrink-0 rounded-full border border-border/50 shadow bg-muted p-0.5">
                <div className="overflow-hidden rounded-full">
                  <div className="h-full w-full bg-slate-6 rounded-full"></div>
                </div>
              </div>
            </div>
          ))}
    ...
    },
    {
      ...
      visual: (
        <div className="relative flex h-full items-center justify-center">
          <div className="h-32 w-full bg-gradient-to-t from-slate-1 via-slate-3 to-slate-1 rounded flex items-center justify-center">
